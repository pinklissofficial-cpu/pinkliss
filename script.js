...
const products = Array.from({ length: 120 }, (_, i) => {
  ...
  const names = {
    Lipstick: ["Velvet Matte Lipstick", "Crystal Shine Lipstick", "Rose Nude Lip Cream"],
    Foundation: ["Radiance Foundation", "Air Touch Foundation", "Luminous Skin Tint"],
    Skincare: ["Glow Serum", "Hydra Moisturizer", "Silk Face Mask"],
    Perfume: ["Noir Bloom Perfume", "Pink Aura Eau de Parfum", "Royal Rose Mist"],
    "Eye Makeup": ["Lash Luxe Mascara", "Midnight Kohl Liner", "Velvet Eyeshadow Quad"],
    Brushes: ["Pro Blend Brush", "Rose Gold Brush Set", "Precision Beauty Brush"],
    "Gift Sets": ["Signature Luxe Set", "Pink Delight Box", "Golden Glow Hamper"]
  };

  return {
    id: i + 1,
    name: `${names[category][i % names[category].length]} ${String(i + 1).padStart(3, "0")}`,
    category,
    price: 499 + (i % 12) * 80,
    desc: `Premium ${category.toLowerCase()} item crafted for a luxury beauty experience.`,
...
function render() {
  ...
  state.page = ["home", "shop", "categories", "checkout", "admin"].includes(route) ? route : "home";
...
function shopView() {
  const filtered = products.filter(p => {
    ...
    const matchesSearch = `${p.name} ${p.category}`.toLowerCase().includes(state.search.toLowerCase());
...
function bindPageEvents() {
  document.querySelectorAll("[data-open-product]").forEach(el => {
    el.addEventListener("click", () => openProductModal(Number(el.dataset.openProduct)));
  });

  document.querySelectorAll("[data-add-to-cart]").forEach(el => {
    el.addEventListener("click", () => addToCart(Number(el.dataset.addToCart)));
  });
  ...
  document.querySelectorAll("[data-category]").forEach(el => {
    el.addEventListener("click", () => {
      ...
      if (state.page === "shop" || state.page === "categories") render();
    });
  });
  ...
  if (checkoutForm) {
    ...
    checkoutForm.addEventListener("submit", (e) => {
      ...
      const order = {
        ...
        items: state.cart.map(item => `${getProduct(item.id).name} x ${item.qty}`).join(", "),
        total: cartTotal()
      };
  ...
  if (closeDetailsBtn) closeDetailsBtn.addEventListener("click", closeProductModal);
}
